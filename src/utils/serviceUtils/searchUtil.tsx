...
class SearchUtil {
  ...
  static mouseSearch(books: BookModel[]) {
    // let keyword = (document.querySelector(".header-search-box") as HTMLInputElement).value.toLowerCase();
  ...
  static keySearch(event: any, books: BookModel[]) {
    if (event && event.keyCode === 13) {
      ...
      // let bookResults = this.fuzzyQuery(bookNameArr, event.target.value.toLowerCase());
      // let authorResults = this.fuzzyQuery(AuthorNameArr, event.target.value.toLowerCase());
  ...
  static keyNoteSearch(event: any, notes: NoteModel[]) {
    if (event && event.keyCode === 13) {
      ...
      // let noteResults = this.fuzzyQuery(noteArr, event.target.value.toLowerCase());
      // let textResults = this.fuzzyQuery(textArr, event.target.value.toLowerCase());
      // return this.mergeArray(noteResults, textResults);
    }
  }
}
