...
import { exportBooks, exportDictionaryHistory, exportHighlights, exportNotes } from "../../utils/syncUtils/exportUtil";
...
class SelectBook extends React.Component<BookListProps, BookListState> {
  ...
  render() {
    return (
      <div className="booklist-manage-container" style={this.props.isCollapsed ? { left: "75px" } : {}}>
        {this.props.isSelectBook && (
          <>
            ...
            {/* <span
              ...
              onClick={() => {
                if (
                  this.props.books.filter((item: BookModel) => this.props.selectedBooks.indexOf(item.key) > -1).length >
                  0
                ) {
                  AddFavorite.setFavorites(
                    this.props.books.filter((item: BookModel) => this.props.selectedBooks.indexOf(item.key) > -1)
                  );
                  this.props.handleSelectBook(!this.props.isSelectBook);
                  if (this.props.isSelectBook) {
                    this.props.handleSelectedBooks([]);
              ...
              >
                ...
                <span
                  ...
                  onClick={async () => {
                    let selectedBooks = this.props.books.filter(
                      (item: BookModel) => this.props.selectedBooks.indexOf(item.key) > -1
                    );
                    if (
                      this.props.notes.filter(
                        (item) =>
                          selectedBooks.filter((subitem) => subitem.key === item.bookKey).length > 0 &&
                          item.notes !== ""
                      ).length > 0
                    ) {
                      exportNotes(
                ...
                <span
                  ...
                  onClick={async () => {
                    ...
                    let dictHistory = (await window.localforage.getItem("words")) || [];
                    dictHistory = dictHistory.filter(
                      (item) => selectedBooks.filter((subitem) => subitem.key === item.bookKey).length > 0
                    );
                  ...
                  onClick={async () => {
                    if (
                      ...
                      for (let index = 0; index < selectedBooks.length; index++) {
                        ...
                        let result = await BookUtil.fetchBook(selectedBook.key, true, selectedBook.path);
            ...
            <span
              className="book-manage-title select-book-action"
              onClick={() => {
                if (this.props.selectedBooks.length === this.handleFilterShelfBook(this.props.books).length) {
                  this.props.handleSelectedBooks([]);
                } else {
                  this.props.handleSelectedBooks(this.handleFilterShelfBook(this.props.books).map((item) => item.key));
                }
              }}
            >
              {this.props.selectedBooks.length === this.handleFilterShelfBook(this.props.books).length ? (
