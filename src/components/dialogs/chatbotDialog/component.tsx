// src/components/dialogs/chatbotDialog/component.tsx
import React, { useState } from "react";
import { withRouter } from "react-router-dom";
import { ChatbotDialogProps, ChatbotDialogState } from "./interface";
import "./chatbotDialog.css";
import "./chatbotWidget.css";
import ChatbotWidget from "./chatbotWidget";
import { XIcon, BotMessageSquareIcon } from "lucide-react";
...
const ChatbotDialog: React.FC<ChatbotDialogProps> = ({
  ...
  const [state, setState] = useState<ChatbotDialogState>({
    isShow: false,
    isLoading: false,
    isChat: false,
    messages: [],
  ...
  return (
    <div className="m-chatbot-container">
      <div className={`chatbot-icon-container ${state.isShow ? "active" : ""}`} onClick={handleShow}>
        {state.isShow ? (
          <XIcon />
        ) : (
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_iconCarrier">
              <g id="🔍-Product-Icons" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                <g className="chatbot-icon" fillRule="nonzero">
                  <path
      ...
      {state.isShow && (
        <div className="chatbot-widget-container">
          <ChatbotWidget handleShow={handleShow} />
        </div>
      )}
    </div>
  );
...
export default withRouter(ChatbotDialog as any);
